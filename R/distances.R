...
DistanceCalculator <- R6::R6Class(
    public = list(
        get_matrix = function(x, distance_type, earth_radius) {
            switch(
                distance_type,
                LINEAR={return(self$calc_linear_dist(x, x))},
                EUCLIDEAN={return(self$calc_euclidean_dist(x, x))},
                HAVERSINE={return(self$calc_haversine_dist(x, x, earth_radius))},
        ...
        check_tensor_dimensions = function(x1, x2, expected_nb_dim, expected_last_dim_shape) {
            if (!(tsr$is_tensor(x1) && tsr$is_tensor(x2, torch.Tensor))){
        ...
        calc_haversine_dist = function(x1, x2, earth_radius = EARTH_RADIUS_KM) {
            ...
            xu1 = x1$unsqueeze(1)
            xu2 = x2$unsqueeze(1)
            xu1 = torch$deg2rad(xu1)
            xu2 = torch$deg2rad(xu2)
